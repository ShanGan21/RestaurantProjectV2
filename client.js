...
//You should remove these for your V2 implementation, as all data should come from the server
// let aragorn = {
...
// let prevAssign = [aragorn, legolas, frodo];
...
// Stores the total order price for an order from a restaurant
...
//For V2, you will likely have to make an XMLHttpRequest from here to retrieve the array of restaurant names.
function genDropDownList(){
	// Let resNames hold all of the restaurant names for the dropdown list
	let resNames;
	let xhttp = new XMLHttpRequest();
	xhttp.onreadystatechange = function() {
		if(this.readyState==4 && this.status==200) {
			resNames = JSON.parse(this.responseText);
	...
	// Sends a xhttp request to the server
	xhttp.open("GET", "/server.js", false);
	xhttp.send();
	
	// Sets each restaurant name
...
//For V2, you will likely have to make an XMLHttpRequest here to retrieve the menu data for the selected restaurant
function selectRestaurant(){
	...
	if(result){
		...
		//In V2, current restaurant will be data you received from the server


		// Let ourRestObjrepresent the restaurant menu object from the server
		// Generate a new XMLHttp Request to obtain this 
		let ourRestObj;
		let xhttp = new XMLHttpRequest();

		xhttp.onreadystatechange = function() {
			if(this.readyState==4 && this.status==200) {
				// Let obj represent the value of the responseText
				obj = JSON.parse(this.responseText);

				// obj contains an object of restaurant
				// If the name is equal to what is wanted, then it sets it as the ourRestObj
				for (let i = 0; i < obj.length; i++) {
					// Checks which restaurant object
		...
		xhttp.open("GET", "/server.js/allRestaurants", false);
		xhttp.send();

		// Sets the currentRestaurant as the ourRestObj
		currentRestaurant = ourRestObj;
...
function getCategoryHTML(rest){
	// Let ourRestMenu represent the restaurant menu object from the server
	...
	xhttp.onreadystatechange = function() {
		if(this.readyState==4 && this.status==200) {
			// This is an array which stores all of the restaurant objects from the server
			obj = JSON.parse(this.responseText);

			// Goes through this array and determines what restaurant object it is (comparing name)
			// Sets it as the restaurant menu object
			for (let i = 0; i < obj.length; i++) {
				// Checks which restaurant object
				if (obj[i].name == rest.name) {
					ourRestMenu = obj[i].menu;
				}
			}
		}
	}
	...
	// Goes through all of the keys of the ourRestMenu (holds the restaurant)
	Object.keys(ourRestMenu).forEach(key =>{
...
function getMenuHTML(rest){
	...
	// Sets menu as the selected restaurant's menu
	let menu = ourRestMenu;
	let result = "";
...
//For V2, you will likely make an XMLHttpRequest here
function submitOrder(){
	alert("Order placed!")

	// jsonOrder represents the JSON object of the order being sent to server
	// orderObj holds the current order
	let jsonOrder;
	let orderObj = {};

	// Sends the name, order, and total price of the order
	orderObj.name = currentRestaurant.name;
	orderObj.orders = order;
	orderObj.totalPrice = totalOrderPrice;

	// Converts to JSON object
	jsonOrder = JSON.stringify(orderObj);

	// Sends XMLHttpRequest to the server
	let xhttp = new XMLHttpRequest();
	...
	xhttp.open("POST", "/server.js/orderplaced", true);
	xhttp.setRequestHeader("Content-Type", "application/json");
	xhttp.send(jsonOrder);

	// Refreshes the order and calls selectRestaurant
	order = {};
	selectRestaurant();
}
